...
async function getCommentAttachments(taskGid, commentGid) {
  ...
  const parent = commentGid || taskGid;
  const res = await fetch(
    `${ASANA_API_BASE}/attachments?parent=${encodeURIComponent(parent)}` +
    '&opt_fields=name,download_url,view_url,resource_subtype,size', { headers });
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
