...
const BILINGUAL_PO_KEY = 'standard-po-v2';
...
function bilingualPoTemplate() {
  ...
  /* One block per order. Deliberately short: the full detail lives on the
   * supplier's order page, and the link to it is appended once by the
   * caller rather than being repeated inside every block. */
  const en = [
  ...
  return {
    ...
    isDefault: true,
    en: { subject: 'Purchase Order {{poNumber}} - {{componentName}}', body: en },
    zh: { subject: '采购订单 {{poNumber}} - {{componentName}}', body: zh },
    createdAt: now,
    updatedAt: now
  };
}
...
function ensureBilingualDefault() {
  // loadAll() returns null when the store has never been seeded, so go
  // through listTemplates() which performs that first-run seeding.
