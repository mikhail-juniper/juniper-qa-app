 ...
 */
/**
 * Product title from an Asana task name.
 *
 * Tasks are named "PONUMBER - Product Title", e.g.
 * "JUPG02PLU1-PO1 - Queen Bee + Bee Plush Keychain Set".
 *
 * Split on the FIRST separator only: product names legitimately contain
 * dashes and plus signs ("Queen Bee + Bee Plush Keychain Set"), so
 * splitting greedily would truncate them. The PO number is stripped by
 * prefix when it matches, which also handles titles that themselves start
 * with something dash-like.
 */
function productTitleFromTaskName(taskName, poNumber) {
  let name = String(taskName || '').trim();
  if (!name) return null;

  // Preferred: the name starts with this PO number, so remove exactly that.
  const po = String(poNumber || '').trim();
  if (po && name.toLowerCase().startsWith(po.toLowerCase())) {
    name = name.slice(po.length).trim();
    // Drop the separator, whichever dash style was used.
    name = name.replace(/^[-\u2013\u2014:]\s*/, '').trim();
    return name || null;
  }

  // Fallback: first " - " style separator, so a PO number we didn't expect
  // still gets stripped rather than ending up in the product title.
...
async function pullFromAsana(poNumber) {
  ...
  return {
    ...
    taskName: task.name || '',
    fields: {
      // Everything after "PONUMBER - " in the Asana task name.
      productTitle: productTitleFromTaskName(task.name, poNumber),
...
function buildPushPayload(order, extras = {}) {
  ...
  /* Deep link back to this PO in the ERP, so someone in Asana can jump
   * straight to it. Proposed Inspection % and QA Check Percentage were
   * dropped - those live in the ERP now and their Asana options never
   * matched the values we produced. */
  if (extras.poLink) put('poLink', extras.poLink);
  // Pass/Rework/etc, not the report URL - the field is a dropdown.
...
/**
 * Per-field write options: which type wins when two Asana fields share a
 * name, and which dropdowns may gain new options automatically.
 */
function fieldWriteOptions() {
  const cfg = loadConfig() || {};
  const opts = {};
  Object.entries(cfg.preferFieldType || {}).forEach(([name, type]) => {
    opts[name] = { ...(opts[name] || {}), preferType: type };
  });
  (cfg.createMissingEnumOptions || []).forEach((name) => {
    opts[name] = { ...(opts[name] || {}), createMissingOption: true };
  });
  return opts;
}
...
/**
 * Push this order's fields to its Asana task.
 *
 * `previous` is the order as it was before the edit. When given, fields whose
 * value has not actually changed are dropped from the payload.
 *
 * This matters more than it looks. Asana automations fire on "field X is
 * changed", and Asana counts a write as a change even when the value written
 * is identical. The detail panel autosaves on every edit, so one sitting with
 * a PO sent several identical writes of the fulfil date and the workspace rule
 * posted the same comment to the task three times over.
 */
async function pushToAsana(order, extras = {}, previous = null) {
  ...
  if (previous) {
    const before = buildPushPayload(previous, extras);
    Object.keys(payload).forEach((k) => {
      ...
      if (same) delete payload[k];
    });
  }

  if (!Object.keys(payload).length) return { ok: false, skipped: 'nothing changed' };
  try {
    return await asanaClient.setFieldsByName(order.asanaTaskGid, payload, fieldWriteOptions());
  } catch (err) {
...
module.exports = {
  productTitleFromTaskName, loadConfig, isConfigured, pullFromAsana, buildPushPayload, pushToAsana };
