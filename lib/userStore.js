 ...
 *
 * Adding another sign-in provider means writing the identity link onto an
...
function normalize(u) {
  return {
    /* UI preferences, so a saved column choice follows the person rather than
       the browser they happened to use. */
    ...
    // Separate grant for Drive: a different scope, requested separately, so
    // revoking one doesn't take the other with it.
    driveRefreshToken: u.driveRefreshToken || null,
...
function publicView(u) {
  if (!u) return null;
  const { passwordHash, gmailRefreshToken, driveRefreshToken, ...rest } = u;
  return {
    ...rest,
    hasPassword: !!passwordHash,
    gmailConnected: !!gmailRefreshToken,
    driveConnected: !!driveRefreshToken
  };
}
...
/** Merge a preferences patch onto a user without touching anything else. */
function setPreferences(id, patch) {
  const u = getUserById(id);
  if (!u) return null;
  return updateUser(id, { preferences: { ...(u.preferences || {}), ...(patch || {}) } });
}
...
function updateUser(id, patch) {
  ...
  entries[idx].driveRefreshToken = merged.driveRefreshToken || null;
  saveAll(entries);
  return entries[idx];
}
...
function redactOrderForSupplier(user, order) {
  ...
  return {
    ...
    /* Manufacturing files the factory needs to actually make the order -
     * the design documents attached on our side. Only these categories are
     * exposed; internal paperwork like packing lists stays private. */
    manufacturingFiles: isMainSupplier
      ? (order.files || [])
        .filter((f) => f && f.url && ['Design document', 'Style picture'].includes(f.category))
        .map((f) => ({ name: f.originalName || '', category: f.category || '', url: f.url }))
      : [],
    componentPhotos: isMainSupplier
      ? (order.accessories || []).map((a) => ({
        partName: a.partName || '',
        imageUrl: a.imageUrl || '',
        // Each sub-component's own design document, so the assembling
        // factory can download the spec for every part.
...
module.exports = {
  ...
  permissionsFor, can, scopeOrdersForUser, canSeeOrder, redactOrderForSupplier, publicView, normalize, setPreferences
};
