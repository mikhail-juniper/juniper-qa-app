...
/**
 * A repeat order inherits everything the last PO for this SKU already knows.
 *
 * The first PO of a product has its photos, drawings and component details
 * linked in Asana. Subsequent POs never do - the Asana task for a PO2 carries
 * the order header and little else - so this is where the rest comes from.
 *
 * Only ever fills BLANKS. Anything that arrived with the new PO, whether typed
 * or synced from Asana, wins: this is a starting point for a reorder, not a
 * correction of what someone just entered.
 *
 * Returns a list of what it filled, for the changelog. Saying "inherited from
 * JDAN01PLU1-PO1" is the difference between trusted data and data someone has
 * to re-check by hand.
 */
function inheritFromPreviousPo(entry, actor) {
  const sku = (entry.mainComponent && entry.mainComponent.sku || '').trim().toLowerCase();
  if (!sku) return [];

  const prior = listOrders({})
    .filter((o) => o.id !== entry.id
      && String((o.mainComponent || {}).sku || '').trim().toLowerCase() === sku)
    .sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')))[0];
  if (!prior) return [];

  const filled = [];
  const mc = entry.mainComponent || (entry.mainComponent = {});
  const pmc = prior.mainComponent || {};

  const copyMain = (key, label) => {
    const has = mc[key] !== undefined && mc[key] !== null && mc[key] !== '';
    if (has || pmc[key] === undefined || pmc[key] === null || pmc[key] === '') return;
    mc[key] = pmc[key];
    filled.push(label);
  };
  copyMain('name', 'product title');
  copyMain('photoReference', 'photo reference');
  copyMain('manufacturingDrawing', 'manufacturing drawing');
  copyMain('washingTagUrl', 'washing tag');
  copyMain('hangTagUrl', 'hang tag');
  copyMain('packagingUrl', 'packaging');
  copyMain('weightGrams', 'weight');
  copyMain('shippingWeightGrams', 'shipping weight');
  copyMain('volumeWeightGrams', 'volume weight');
  copyMain('warehouse', 'warehouse');

  /* Costs are their own object on the order, not a main-component field. A
     repeat run usually starts from last time'''s numbers and gets negotiated
     from there, so they carry across only when this PO has none. */
  const zeroCosts = !entry.costs || Object.values(entry.costs).every((v) => !v);
  const priorHasCosts = prior.costs && Object.values(prior.costs).some((v) => v);
  if (zeroCosts && priorHasCosts) {
    entry.costs = { ...prior.costs };
    filled.push('costs');
  }

  if (!mc.docSourceUrls || !Object.keys(mc.docSourceUrls).length) {
    if (pmc.docSourceUrls && Object.keys(pmc.docSourceUrls).length) {
      mc.docSourceUrls = { ...pmc.docSourceUrls };
      filled.push('document links');
    }
  }
  if (!mc.dimensions || !(mc.dimensions.height || mc.dimensions.width || mc.dimensions.depth)) {
    if (pmc.dimensions && (pmc.dimensions.height || pmc.dimensions.width || pmc.dimensions.depth)) {
      mc.dimensions = { ...pmc.dimensions };
      filled.push('dimensions');
    }
  }

  /* Sub-components come across without their quantities or ids: the parts are
   * the same, how many of each is a decision for this run. */
  if (!(entry.accessories || []).length && (prior.accessories || []).length) {
    entry.accessories = prior.accessories.map((a) => ({
      ...a,
      id: uuidv4(),
      quantity: null,
      dispatchedAt: null
    }));
    filled.push(`${entry.accessories.length} sub-component(s)`);
  }

  if (!entry.supplier || !entry.supplier.name) {
    if (prior.supplier && prior.supplier.name) {
      entry.supplier = { ...prior.supplier };
      filled.push('supplier');
    }
  }
  if (!entry.category && prior.category) { entry.category = prior.category; filled.push('category'); }
  if (!entry.subcategory && prior.subcategory) { entry.subcategory = prior.subcategory; }
  if (!entry.creator && prior.creator) { entry.creator = prior.creator; filled.push('creator'); }
  if (!entry.productDevelopmentLead && prior.productDevelopmentLead) {
    entry.productDevelopmentLead = prior.productDevelopmentLead; filled.push('PD lead');
  }
  if (!entry.sourcer && prior.sourcer) { entry.sourcer = prior.sourcer; filled.push('sourcer'); }

  if (filled.length) {
    entry.inheritedFromPoNumber = prior.poNumber;
    logChange(entry, actor || 'Previous PO', 'Inherited from previous PO',
      `${prior.poNumber}: ${filled.join(', ')}`);
  }
  return filled;
}
...
function normalizeAccessory(a) {
  ...
  return {
    id: a.id || uuidv4(),
    /* Sub-components are made by their own factory on their own schedule, so
     * a call about the hang tag is a different conversation from one about the
     * plush. Notes left against a component belong to that component, not
     * folded into the parent PO's log where they lose their thread. */
    progressLog: Array.isArray(a.progressLog) ? a.progressLog : [],
...
function createOrder(data, actor) {
  ...
  /* Before the component library fills doc slots, take everything the last PO
   * for this SKU already established. Blanks only. */
  inheritFromPreviousPo(entry, actor);
 ...
 */
/** Append a progress note to one sub-component's own log. */
function addAccessoryProgressNote(id, accessoryId, text, actor) {
  const clean = String(text || '').trim();
  if (!clean) return null;
  const only = orderDb.getOne(id);
  if (!only) return null;
  const order = hydrateOrder(only);
  const accessories = order.accessories || [];
  const idx = accessories.findIndex((a) => a.id === accessoryId);
  if (idx === -1) return null;

  accessories[idx] = {
    ...accessories[idx],
    progressLog: [
      ...(Array.isArray(accessories[idx].progressLog) ? accessories[idx].progressLog : []),
      { text: clean, at: new Date().toISOString(), by: actor || 'Web user' }
    ]
  };
  order.accessories = accessories;
  order.updatedAt = new Date().toISOString();
  logChange(order, actor, `Progress note (${accessories[idx].partName || 'sub-component'})`,
    clean.slice(0, 80));
  saveOne(order);
  return order;
}
...
module.exports = {
  ...
  listOrders, updateOrder, setStatus, inheritFromPreviousPo, setSettlement, deleteOrder, addProgressNote, addAccessoryProgressNote, advanceOnPdApproval, advanceOnMainPoDispatch, advanceAccessoryOnDispatch, addFile, removeFile, logNote, applyDefinitionsToDocSlots, logNote, listSuppliers, listProducts,
