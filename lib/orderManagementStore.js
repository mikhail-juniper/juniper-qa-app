...
function nextActionFor(order, approvalStatuses, today) {
  ...
  const stageOf = (k) => (approvalStatuses && approvalStatuses[k]) || 'Not Started';
  ...
  // 3. PD approval: submitted work that nobody has decided on.
  for (const [key, label] of [['sampleApproval', 'Golden Sample'], ['preProductionApproval', 'Pre-Production'], ['bulkApproval', 'Bulk']]) {
    const st = stageOf(key);
    if (st === 'Submitted' || st === 'Awaiting PD') {
      return { owner: 'pd', kind: 'pdReview', stage: key, dueDate: null,
        label: `${label} approval waiting on Product Development`, overdueBy: 0 };
    }
    if (st === 'Changes Requested' || st === 'Commented') {
      return { owner: 'you', kind: 'pdReply', stage: key, dueDate: null,
        label: `${label} approval needs your reply`, overdueBy: 0 };
    }
  }
