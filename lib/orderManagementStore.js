...
/**
 * Record the fields a factory maintains itself. Deliberately separate from
 * updateOrder: this is the ONLY write a supplier session can perform, and
 * it accepts nothing outside these three keys, so a supplier can never
 * reach quantities, prices or status through it.
 */
function setFactoryUpdates(id, patch, actor) {
  const entries = loadAll();
  const idx = entries.findIndex((e) => e.id === id);
  if (idx === -1) return null;
  const allowed = ['preProductionSampleDate', 'bulkSampleDate'];
  ...
  allowed.forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(patch || {}, key)) {
      ...
      if (String(next[key] || '') !== String(value || '')) changed.push(key);
      next[key] = value;
    }
  });

  /* Bulk progress is append-only: `bulkProgressNote` adds a dated entry
   * rather than overwriting. Deliberately not editable or deletable - it's
   * a record of what the factory reported and when, so it shouldn't be
   * rewritable after the fact. */
  const note = typeof patch.bulkProgressNote === 'string' ? patch.bulkProgressNote.trim() : '';
  if (note) {
    next.bulkProgressLog = [
      ...(Array.isArray(next.bulkProgressLog) ? next.bulkProgressLog : []),
      { text: note, at: new Date().toISOString(), by: actor || 'Supplier' }
    ];
    changed.push('bulk progress update');
  }
  if (!changed.length) return entries[idx];
  next.updatedAt = new Date().toISOString();
  next.updatedBy = actor || 'Supplier';
  entries[idx].factoryUpdates = next;
  entries[idx].updatedAt = next.updatedAt;
  logChange(entries[idx], actor || 'Supplier', 'Factory update', changed.join(', '));
  saveAll(entries);
  return entries[idx];
}
...
module.exports = {
  ...
  listOrders, updateOrder, setStatus, setSettlement, deleteOrder, setFactoryUpdates, advanceOnPdApproval, advanceOnMainPoDispatch, advanceAccessoryOnDispatch, addFile, removeFile, listSuppliers, listProducts,
