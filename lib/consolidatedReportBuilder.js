...
async function buildConsolidatedReport(po, approval, reportingHistory, i18n, revisedReports) {
   ...
   * Appended last so the merged document reads in the order things happened:
   * what was found, then what was repaired afterwards. They are a separate
   * section rather than folded into the inspection reports above, because the
   * original report's findings are the record of the inspection and should not
   * be rewritten by a later repair. */
  if ((revisedReports || []).length) {
    const revisedBuffer = await buildSectionPdf(i18n, (pdf, doc) => {
      pdf.drawHeader({ poNumber: po.poNumber, _reportId: 'REVISED' });
      pdf.sectionTitle(null, bilingual('Revised Unit Reports', '返修确认报告'));
      revisedReports.forEach((rep, idx) => {
        const when = rep.submittedAt ? String(rep.submittedAt).slice(0, 10) : '-';
        const stage = rep.qaType === 'production' ? 'Bulk' : 'Pre-Production';
        pdf.subheading(`${bilingual('Revised Unit Report', '返修确认报告')} #${idx + 1} - ${stage} - ${when}`);
        (rep.issues || []).forEach((iss) => {
          pdf.paragraph(
            `${iss.description || ''} (${iss.unitsAffected || 0} ${bilingual('units flagged', '件标记')})`,
            `${iss.unitsFixed || 0} ${bilingual('repaired', '件已修复')}`
          );
        });
        const fixed = (rep.issues || []).reduce((n, i) => n + (parseInt(i.unitsFixed, 10) || 0), 0);
        const flagged = (rep.issues || []).reduce((n, i) => n + (parseInt(i.unitsAffected, 10) || 0), 0);
        pdf.keyValueGrid([
          [bilingual('Units repaired', '已修复件数'), `${fixed} / ${flagged}`],
          [bilingual('Confirmed by', '确认人'), rep.qaLead || '-']
        ], 2);
        doc.moveDown(0.5);
      });
    });
    buffers.push(revisedBuffer);
  }
