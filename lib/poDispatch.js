...
function buildTargets(order) {
  ...
  targets.push({
    ...
    photoReference: (order.mainComponent && order.mainComponent.photoReference) || ''
  });
  ...
  (order.accessories || []).forEach((a) => {
    ...
    targets.push({
      ...
      /* The sub-component's own reference image. A hang-tag supplier needs to
       * see the hang tag, not the plush it goes on - sending the product shot
       * told them nothing about what they were being asked to make. Empty when
       * nothing has been attached to the row; no fallback to the product photo
       * on purpose, since a wrong reference is worse than none. */
      photoReference: a.imageUrl || ''
    });
  });
 ...
 */
/**
 * The default bilingual PO message.
 *
 * `portalLink` is the supplier's own order page. It's included because the
 * message is how a factory gets in - there's no login, so without the link
 * in the message they have no route to the order details, drawings or
 * sizing chart.
 */
function buildMessage(order, target, portalLink) {
  const lines = [];
  ...
  lines.push('');
  if (portalLink) {
    // The blank line before this block is already pushed above.
    lines.push('在线查看此订单 / View this order online:');
 ...
 */
function templateValues(order, target, portalLink, sender) {
  ...
  return {
    ...
    // Who is sending, so the factory has a named person to reply to rather
    // than a generic company signature.
 ...
 */
function buildLogEntry(target, channel, recipient, actor, sender) {
  return {
    // Who it went out as - distinct from `sentBy`, which is just the label
    // of whoever was operating the app.
