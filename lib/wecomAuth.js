...
async function getJson(url) {
  ...
  if (body.errcode) {
    // 60020 means this server's outbound IP isn't on the app's Trusted IP
    // list. WeCom helpfully includes the IP it saw, so pull it out and say
    // plainly what to add - Render's outbound address comes from a shared
    // pool and can change on a redeploy, so this WILL recur.
    if (String(body.errcode) === '60020') {
      const seen = String(body.errmsg || '').match(/from ip:\s*([0-9.]+)/);
      const ip = seen ? seen[1] : 'this server\'s IP';
      const err = new Error(
        `WeCom is blocking this server's IP. Add ${ip} to the app's ` +
        'Trusted IPs (企业可信IP) in the WeCom admin console, then try again.');
      err.wecomIp = seen ? seen[1] : null;
      err.errcode = 60020;
      throw err;
    }
    const err = new Error(`WeCom error ${body.errcode}: ${body.errmsg || 'unknown'}`);
