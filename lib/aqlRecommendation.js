...
function getRecommendation({ category, subcategory, poQuantity, creator, risk, sku }, { unitCosts, aqlRecConfig, creatorTiersConfig, findOrdersBySku }) {
  /* Returns a recommendation, or { unavailable: true, reason } explaining
   * why it can't. It used to return a bare null on any missing input, so
   * the whole section silently vanished and there was no way to tell a bug
   * from an incomplete PO - which is exactly what happened in practice. */
  const fail = (reason) => ({ unavailable: true, reason });

  if (!aqlRecConfig || !aqlRecConfig.table || !aqlRecConfig.poSizeBands) {
    return fail('missingConfig');
  }
  if (!creatorTiersConfig) return fail('missingCreatorTiers');
  ...
  if (!tierTable || !tierTable[riskKey] || !tierTable[riskKey][poSizeBand]) {
    return fail('noTableEntry');
  }
  ...
  return {
    ...
    poSizeBand,
    tier,
    // Whether the value came from a real PO price or the estimate table -
    // useful when the recommendation looks surprising.
    costSource: realCost !== null ? 'poPrice' : 'estimate',
