...
function init(dataDir) {
  ...
  /* See the same guard in lib/jsonRowDb.js - a full or read-only disk shows
   * up here as an opaque SqliteError on an empty database. */
  try {
    db.pragma('journal_mode = WAL');
    db.pragma('synchronous = NORMAL');
  } catch (err) {
    if (err && err.code === 'SQLITE_FULL') {
      throw new Error(`Cannot open ${dbPath}: the disk is full. Free space on the `
        + `persistent disk holding DATA_DIR, then redeploy. Original error: ${err.message}`);
    }
    if (err && (err.code === 'SQLITE_READONLY' || err.code === 'SQLITE_CANTOPEN')) {
      throw new Error(`Cannot open ${dbPath}: not writable. Check DATA_DIR points at the `
        + `mounted persistent disk. Original error: ${err.message}`);
    }
    throw err;
  }
