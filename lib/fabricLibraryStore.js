...
function createFabricCode(data) {
  ...
  const entry = {
    ...
    hex: data.hex || '',
    cmyk: data.cmyk || '',
    // The factory's own reference for the colour book (HSW227, MLD 1MM,
    // WLD100C ...). This is what a factory needs to identify a colour -
    // the Pantone alone isn't enough for them to pull the right swatch.
