...
function createSupplier(data) {
  ...
  const entry = {
    ...
    wechat: data.wechat || '',
    // Contact channels used when dispatching a PO to this factory. email is
    // new; wechat above already existed for reference and is reused here.
    email: data.email || '',
    // Per-supplier access link token (lib/supplierAccess). Lets a factory
    // open their own PO page without an account.
    accessToken: data.accessToken || '',
    accessTokenIssuedAt: data.accessTokenIssuedAt || null,
