...
/* ---- Config-driven inspection rendering (Steps 5 and 6) ----
 * Everything here is read straight off payload.inspection, which the report
 * app sends with question titles already resolved. We deliberately do NOT look
 * titles up from config/reportQuestions.json at render time: a report is an
 * audit document, and re-resolving it against a config that has since changed
 * would quietly rewrite what the inspector was asked. */

function inspectionSections(pdf, payload, filesByField, i18n) {
  const sections = (payload.inspection && payload.inspection.sections) || [];
  if (!sections.length) return;

  sections.forEach((section) => {
    pdf.sectionTitle(null, section.name);
    section.questions.forEach((q) => {
      const label = q.status === 'fail' && q.unitsAffected
        ? `${q.title}  (${q.unitsAffected} ${bi(i18n, 'unitsAffectedShort') || 'units'})`
        : q.title;
      pdf.checklistRow(label, q.status || 'na', '');

      // Evidence sits directly under the question it belongs to, so a reader
      // never has to match a photo back to a check.
      const photos = (filesByField[`photo_q_${q.id}`] || []).map((f) => ({ buffer: f.buffer }));
      if (photos.length) pdf.photoGridSync(photos, 4);
    });
  });
}

/**
 * How a defect was resolved, for printing beside it.
 *
 * Without this the PDF showed the original finding with no indication it had
 * been dealt with - so a defect repaired on the factory floor read to the
 * factory as an open, unaddressed problem, and the numbers in the recap
 * wouldn't reconcile with the list above them.
 */
function resolutionLabel(payload, defectId, i18n) {
  const list = (payload.inspection && payload.inspection.dispositions) || [];
  const d = list.find((x) => x.id === defectId);
  if (!d || !d.choice) return '';
  if (d.choice === 'repaired') {
    return `${bi(i18n, 'disposition_repaired')} (${d.unitsRepaired || 0}/${d.unitsFlagged || 0})`;
  }
  if (d.choice === 'rejected') return bi(i18n, 'disposition_rejected');
  return bi(i18n, 'disposition_factory');
}

/** The resolution summary block, printed after the issue lists. */
function dispositionSection(pdf, payload, i18n) {
  const list = (payload.inspection && payload.inspection.dispositions) || [];
  const decided = list.filter((d) => d.choice);
  if (!decided.length) return;

  pdf.sectionTitle('dispositionTitle', 'Defective Units');
  pdf.doc.font('Regular').fontSize(9).fillColor(BRAND.muted)
    .text(bi(i18n, 'dispositionHelp') || '', { width: pdf.pageWidth });
  pdf.doc.moveDown(0.4);

  /* Printed as label/value rather than with a Pass/Fail badge: a row reading
   * "Rejected ... Pass" is actively confusing, and a disposition is not a
   * verdict on the units, it is what was done with them. */
  decided.forEach((d) => {
    pdf.paragraph(
      `${d.description || ''}  (${d.unitsFlagged || 0} ${bi(i18n, 'unitsAffectedShort') || 'units'})`,
      resolutionLabel(payload, d.id, i18n)
    );
  });

  /* Spelled out rather than implied: a reader needs to know that the counts in
   * the recap are after these decisions, not before. */
  const anyFactory = decided.some((d) => d.choice === 'factory');
  if (anyFactory) {
    pdf.doc.font('Bold').fontSize(9).fillColor(BRAND.warn || BRAND.muted)
      .text(bi(i18n, 'pdfFactoryFixNote')
        || 'Units the factory will repair remain counted. A Revised Unit Report is required to confirm the repair.',
        { width: pdf.pageWidth });
    pdf.doc.font('Regular').fillColor(BRAND.text).moveDown(0.4);
  }
}

function issueSections(pdf, payload, filesByField, i18n) {
  const sections = (payload.inspection && payload.inspection.issueSections) || [];
  if (!sections.length) return;

  pdf.sectionTitle('additionalIssuesSection', 'Additional Issues');
  const withIssues = sections.filter((s) => (s.issues || []).length);

  if (!withIssues.length) {
    pdf.doc.font('Regular').fontSize(10).fillColor(BRAND.tealDark).text(bi(i18n, 'noIssues') || 'No issues found.');
    pdf.doc.moveDown(0.5);
    return;
  }

  withIssues.forEach((section) => {
    pdf.subheading(section.name);
    section.issues.forEach((issue) => {
      pdf.defectCard(issue, filesByField);
      const res = resolutionLabel(payload, issue.id, i18n);
      if (res) {
        pdf.doc.font('Bold').fontSize(9).fillColor(BRAND.tealDark)
          .text(`${bi(i18n, 'resolutionLabel') || 'Resolution'}: ${res}`, { width: pdf.pageWidth });
        pdf.doc.font('Regular').fillColor(BRAND.text).moveDown(0.3);
      }
    });
  });

  // Naming the clean sections matters: it's the difference between "we didn't
  // find anything here" and "nobody looked".
  const clean = sections.filter((s) => !(s.issues || []).length).map((s) => s.name);
  if (clean.length) {
    pdf.doc.font('Regular').fontSize(9).fillColor(BRAND.muted || BRAND.tealDark)
      .text(`${bi(i18n, 'checkedNoIssues') || 'Checked with no issues found'}: ${clean.join(', ')}`,
        { width: pdf.pageWidth });
    pdf.doc.moveDown(0.5);
  }
}
...
async function buildPdf(payload, filesByField, fitsConfig, i18n, overallResult, categoriesConfig, recommendation, establishedSizing) {
  return new Promise((resolve, reject) => {
    try {
      ...
      /* --- Inspection Details (Step 5) and Additional Issues (Step 6) ---
       * Both are config-driven now and arrive pre-resolved on payload.inspection.
       * The fixed fabric/embroidery/printing/washTag/packaging blocks that used
       * to live here were replaced when Step 5 moved to the question bank. */
      if (payload.inspection && (payload.inspection.sections || []).length) {
        inspectionSections(pdf, payload, filesByField, i18n);
      } else {
        // A report submitted before the question-bank rework, replayed from the
        // archive. Fall back to the old fixed sections so old PDFs still build.
      ...
      /* Sizing is Step 4 now but still renders here, after the measurement
       * charts above. Standard apparel sizing is already fully captured by
       * those charts, so it only contributes its photos; custom sizing keeps
       * its manual Pass/Fail record. */
      ...
      // --- Additional Issues (Step 6), one block per section ---
      if (payload.inspection && (payload.inspection.issueSections || []).length) {
        issueSections(pdf, payload, filesByField, i18n);
      } else {
        pdf.sectionTitle('additionalIssuesSection', 'Additional Issues');
      ...
      dispositionSection(pdf, payload, i18n);
      ...
      /* Whole-PO assumption. The counts are what the inspector actually found
       * in the sample; the assumption scales that rate across the order. It is
       * an estimate from a sample, so it's labelled as one - the figures come
       * from the report app rather than being recalculated here, so the PDF
       * can't disagree with the screen the inspector signed off. */
      const exRecap = payload.inspection && payload.inspection.recap;
      if (exRecap && exRecap.unitsChecked && exRecap.poQuantity) {
        const pct = (v) => (v === null || v === undefined || isNaN(v)
          ? '-' : `${v >= 10 || v === 0 ? Math.round(v) : Math.round(v * 10) / 10}%`);
        pdf.subheading(bi(i18n, 'totalPoAssumption') || 'Total PO assumption');
        doc.font('Regular').fontSize(9).fillColor(BRAND.muted).text(
          `${bi(i18n, 'recapBasisNote') || 'Scaled from the sample across the whole PO.'} `
          + `${exRecap.unitsChecked} / ${exRecap.poQuantity} `
          + `${bi(i18n, 'recapChecked') || 'checked'} `
          + `(${pct((exRecap.unitsChecked / exRecap.poQuantity) * 100)}).`,
          { width: pdf.pageWidth });
        doc.moveDown(0.3);
        pdf.keyValueGrid(['critical', 'major', 'minor'].map((sev) => {
          const e = exRecap.severities[sev] || {};
          const found = `${e.found || 0}${e.pct !== null && e.pct !== undefined ? ` (${pct(e.pct)})` : ''}`;
          const assumed = e.assumed === null || e.assumed === undefined ? '-' : String(e.assumed);
          return [bi(i18n, 'aql' + sev.charAt(0).toUpperCase() + sev.slice(1)) || sev,
            `${bi(i18n, 'foundLabel') || 'Found'} ${found}  ·  ${bi(i18n, 'totalPoAssumption') || 'Assumption'} ${assumed}`];
        }), 1);
      }

      // --- Video attachments: a PDF can't play video, so each clip is
      // archived server-side and listed here as a clickable link that
      // opens in the browser's own player. ---
      const videoAttachments = (payload._videoAttachments || []);
      if (videoAttachments.length) {
        pdf.sectionTitle('videoAttachmentsTitle', 'Video Attachments');
        doc.font('Regular').fontSize(9.5).fillColor(BRAND.muted)
          .text('Tap a link to play the video in your browser.');
        doc.moveDown(0.4);
        videoAttachments.forEach((v) => {
          pdf.ensureSpace(18);
          const label = `▶  ${v.label || 'Video'}`;
          doc.font('Bold').fontSize(10).fillColor(BRAND.tealDark)
            .text(label, { link: v.absoluteUrl || v.url, underline: true });
          doc.moveDown(0.25);
        });
        doc.moveDown(0.4);
      }
