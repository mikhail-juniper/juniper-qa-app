...
class ReportPDF {
  ...
  aqlSummary(aql, i18n, recommendation, payload) {
    const doc = this.doc;
    this.sectionTitle('aqlReferenceTitle', 'AQL Sampling Reference');
    ...
    if (aql.isFallback) {
      ...
      this.keyValueGrid([
        [bi(i18n, 'poQuantity'), String(aql.lotSize)],
        [bi(i18n, 'inspectionLevel'), 'General ' + aql.inspectionLevel],
        [bi(i18n, 'aqlCodeLetter'), aql.codeLetter],
        [bi(i18n, 'aqlSampleSize'), String(aql.sampleSize)],
      ], 4);
    }
    ...
    if (recommendation) {
      ...
      this.subheading(bi(i18n, 'aqlRecommendationTitle'));
      this.keyValueGrid([
        ...
        [bi(i18n, 'recommendedInspectionLevel'), 'General ' + recommendation.inspectionLevel],
        [bi(i18n, 'recommendedPointCheck'), recommendation.pointCheck],
        [bi(i18n, 'actualSpotCheckPercent'), payload && payload.actualSpotCheckPercent ? payload.actualSpotCheckPercent : '-'],
      ], 3);
    }

    // Defect tally vs thresholds table
    const rows = aql.isFallback
      ? [
          [bi(i18n, 'aqlCritical'), aql.criticalCount, '-', '-'],
          [bi(i18n, 'aqlMajor'), aql.majorCount, '-', '-'],
          [bi(i18n, 'aqlMinor'), aql.minorCount, '-', '-'],
        ]
      : [
          [bi(i18n, 'aqlCritical'), aql.criticalCount, aql.critical.ac, aql.critical.re],
          [`${bi(i18n, 'aqlMajor')} (AQL ${aql.majorAql})`, aql.majorCount, aql.major.ac, aql.major.re],
          [`${bi(i18n, 'aqlMinor')} (AQL ${aql.minorAql})`, aql.minorCount, aql.minor.ac, aql.minor.re],
        ];

    const colPx = [this.pageWidth * 0.4, this.pageWidth * 0.2, this.pageWidth * 0.2, this.pageWidth * 0.2];
    const headerH = 20;
    ...
    let x = PAGE_MARGIN;
    ['', bi(i18n, 'defectTally'), bi(i18n, 'aqlAccept'), bi(i18n, 'aqlReject')].forEach((label, i) => {
      doc.text(label, x + 6, y + 6, { width: colPx[i] - 8, align: i === 0 ? 'left' : 'center' });
      x += colPx[i];
    });
    y += headerH;

    rows.forEach(([label, found, ac, re]) => {
      const rowH = 20;
      this.ensureSpace(rowH);
      if (doc.y !== y) y = doc.y;
      const exceeded = typeof ac === 'number' && found > ac;
      if (exceeded) doc.rect(PAGE_MARGIN, y, this.pageWidth, rowH).fill(BRAND.failBg);
      doc.rect(PAGE_MARGIN, y, this.pageWidth, rowH).strokeColor(BRAND.border).lineWidth(0.5).stroke();
      x = PAGE_MARGIN;
      doc.font('Regular').fontSize(8.5).fillColor(BRAND.text).text(label, x + 6, y + 5, { width: colPx[0] - 8 });
      x += colPx[0];
      doc.font(exceeded ? 'Bold' : 'Regular').fillColor(exceeded ? BRAND.fail : BRAND.text)
        .text(String(found), x, y + 5, { width: colPx[1], align: 'center' });
      x += colPx[1];
      doc.font('Regular').fillColor(BRAND.muted).text(String(ac), x, y + 5, { width: colPx[2], align: 'center' });
      x += colPx[2];
      doc.text(String(re), x, y + 5, { width: colPx[3], align: 'center' });
      y += rowH;
    });
...
function checklistSection(pdf, sectionTitleKey, rows, filesByField, i18n) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (categoriesConfig && categoriesConfig.categories[payload.category]) {
        ...
        categoryLabel = `${catDef.label_en} ${catDef.label_zh}`;
        if (payload.subcategory) {
          const subDef = (catDef.subcategories || []).find((s) => s.key === payload.subcategory);
          if (subDef) subcategoryLabel = `${subDef.label_en} ${subDef.label_zh}`;
        }
      }
      ...
      // --- Checklist sections, each with defects (+ their photos) logged inline ---
      ...
      if (payload.category === 'apparel') {
        // Apparel sizing is already fully captured by the reference + measurement
        // charts above, so skip the checklist and just show the dedicated photos.
        ...
        checklistSection(pdf, 'sizingSection', sizingRows, filesByField, i18n);
      }
