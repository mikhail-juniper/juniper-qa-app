...
/* Backed by SQLite - see lib/jsonRowDb.js. Roughly four parts per SKU means
 * ~18,800 records at a 4,700-PO history: 9.8 MB and ~88 ms per edit on flat
 * JSON, and definitions are written on every PO save via syncComponentDefinitions. */
const { createRowStore } = require('./jsonRowDb');
ensureDir();
const db = createRowStore({
  dataDir: DATA_DIR,
  file: 'componentDefinitions.db',
  table: 'component_definitions',
  idOf: (e) => e.key,
  columns: { sku: (e) => (e.sku ? String(e.sku).trim().toLowerCase() : null) },
...
/** Write one definition instead of all 18,800. */
...
function upsertFromAccessory(sku, accessory, actor) {
  ...
  if (!def) {
    def = {
      ...
      // The bare type ("Hang Tag") behind the qualified partName ("Test Plush
      // Hang Tag"). The doc-slot matchers test against this.
      partType: String(accessory.partType || accessory.partName).trim(),
      createdAt: now,
      updatedAt: now,
  ...
  let changed = false;
  ['partName', 'partType'].forEach((f) => {
    const incoming = accessory[f];
    if (incoming && String(incoming).trim() && String(incoming).trim() !== String(def[f] || '')) {
      def[f] = String(incoming).trim();
      changed = true;
    }
  });
...
function deleteDefinition(id) {
  // Definitions are keyed by sku::partName, so find the row by its id field.
  const hit = loadAll().find((d) => d.id === id);
  if (!hit) return false;
  return db.remove(hit.key);
}
...
module.exports = {
  SPEC_FIELDS, STORE_PATH, keyFor, checkpointDatabase,
...
try {
  ...
  console.error('COMPONENT DEFINITION MIGRATION FAILED - definitions are empty. '
    + 'The JSON file is untouched; fix this before saving a PO.', err);
}
