...
/**
 * Full AQL plan based on the ACTUAL number of units inspected (PO Quantity x
 * Actual Spot Check %), used once that real figure is known - this is what
 * should determine pass/fail, not the theoretical/recommended sample size.
 */
function computeActualAqlPlan({ poQuantity, actualSpotCheckPercent, majorAql = 2.5, minorAql = 4.0 }) {
  const qty = parseInt(poQuantity, 10);
  const pct = parseFloat(String(actualSpotCheckPercent).replace('%', ''));
