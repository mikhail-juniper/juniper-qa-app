...
function computeOverallResult(payload, fitsConfig, establishedSizing) {
  ...
  const applyThresholds = (checked) => {
    ...
    const isCritical = counts.totalDefectiveUnits >= checked;
    const rates = { critical: isCritical ? 100 : 0, major: pct(counts.major.defectiveUnits), minor: pct(counts.minor.defectiveUnits) };
