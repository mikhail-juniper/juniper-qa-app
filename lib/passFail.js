...
function computeOverallResult(payload, fitsConfig) {
  ...
  const plan = payload.poQuantity ? computeAqlPlan({
    lotSize: payload.poQuantity,
    inspectionLevel: payload.inspectionLevel || 'II',
  ...
  if (plan) {
    ...
    // Fallback heuristic - no valid lot size / AQL plan available yet.
