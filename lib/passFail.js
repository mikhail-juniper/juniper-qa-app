 ...
 * unit-by-unit ledger.
 */
function computeQuantityRecap(quantityChecked, majorCount, criticalCount) {
...
function computeOverallResult(payload, fitsConfig) {
  ...
  if (payload.qaType === 'pre_production') {
    ...
    const actualPlan = payload.actualUnitsChecked ? computeActualAqlPlan({
      actualUnitsChecked: payload.actualUnitsChecked,
    ...
    if (actualPlan) {
      ...
      const recap = computeQuantityRecap(payload.actualUnitsChecked, majorCount, criticalCount);
