...
function collectAllDefects(payload) {
  ...
  /* The pre-rework fixed checklist is gone. Scoring only ever runs at submit
   * time against a live payload - stored submissions keep their filed result
   * and PDF and are never re-scored - so the only way an old-shape payload
   * could arrive is a browser tab left open across the deploy.
   *
   * Rather than carry the dead branch, that case is detected and logged. A
   * report scored as clean because its defects were in a shape nobody reads
   * any more is the failure worth being loud about. */
  const legacyDefects = CHECKLIST_KEYS
    .reduce((n, key) => n + (((cd[key] || {}).defects || []).length), 0);
  if (legacyDefects && !(inspection.sections || []).length) {
    console.error(`Submission ${payload.poNumber}: ${legacyDefects} defect(s) arrived in the `
      + 'pre-rework format and were NOT scored. The report page was almost certainly open '
      + 'across a deploy. Ask the inspector to reload and resubmit.');
  }
