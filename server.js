...
const qaDraftStore = require('./lib/qaDraftStore');
...
/* ---- In-progress QA report photos ----
 * Uploaded as they are taken rather than held in the browser until submit, so
 * a half-finished report can be saved and resumed without losing them. See
 * lib/qaDraftStore.js. */
qaDraftStore.init(submissionLog.DATA_DIR);
app.use('/qa-draft-files', express.static(qaDraftStore.root));
app.use('/revised-photos', express.static(submissionLog.REVISED_PHOTO_DIR));

const uploadDraftPhoto = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 60 * 1024 * 1024 }
});

app.post('/api/qa/draft/:draftId/photo', uploadDraftPhoto.single('photo'), (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No photo uploaded' });
    const ref = qaDraftStore.savePhoto(req.params.draftId, req.file);
    if (!ref) return res.status(400).json({ error: 'Invalid draft id' });
    res.json({ ok: true, photo: ref });
  } catch (err) {
    console.error('Draft photo upload failed:', err);
    res.status(500).json({ error: 'Could not save the photo' });
  }
});

/* Save / resume an in-progress report. The draft id is derived from the PO and
 * stage on the client, so reopening the same report link finds the same draft
 * rather than starting a fresh one. */
app.put('/api/qa/draft/:draftId/state', (req, res) => {
  try {
    const ok = qaDraftStore.saveState(req.params.draftId, req.body || {});
    if (!ok) return res.status(400).json({ error: 'Invalid draft id' });
...
app.get('/api/qa/draft/:draftId/state', (req, res) => {
  const found = qaDraftStore.loadState(req.params.draftId);
  res.json({ ok: true, draft: found });
});

app.delete('/api/qa/draft/:draftId', (req, res) => {
  res.json({ ok: qaDraftStore.removeDraft(req.params.draftId) });
});

app.delete('/api/qa/draft/:draftId/photo/:photoId', (req, res) => {
  const ok = qaDraftStore.deletePhoto(req.params.draftId, req.params.photoId);
  res.json({ ok });
});

/* ---- Category consistency check ----
 * The category list is enumerated in several places that cannot import each
 * other: config files, CATEGORY_ORDER in public/app.js, TOP_LEVEL_CATEGORIES in
 * lib/analytics.js. Adding a category means touching all of them, and only some
 * fail loudly - Paper Goods was added to categories.json and silently absent
 * from category analytics for weeks because nothing complained.
 *
 * This logs a warning at boot rather than throwing: a mismatch is a reporting
 * gap, not a reason to refuse to start. */
try {
  const known = Object.keys(categories.categories || {});
  const warn = (where, missing) => {
    if (missing.length) {
      console.warn(`Category mismatch - ${where} is missing: ${missing.join(', ')}. `
        + 'Add it there, or that category will be silently absent.');
    }
  };
  /* require()d here rather than using SHIPPED_TOLERANCES, which is declared
   * further down the file - the same temporal-dead-zone trap that once left
   * the order migration silently doing nothing. require() is cached, so this
   * is the same object, just reachable at this point. */
  const shippedTol = require('./config/tolerances.json');
  warn('config/tolerances.json', known.filter((c) => !(shippedTol.categories || {})[c]));
  warn('config/conditionalChecks.json', known.filter((c) => !(conditionalChecks.byCategory || {})[c]));
  const questionCats = new Set(Object.values(reportQuestions.groups || {}).map((g) => g.category));
  warn('config/reportQuestions.json', known.filter((c) => !questionCats.has(c)));
  // 'other' is priced by unitCosts.otherCategoryFlat rather than a category entry.
  const costCats = (require('./config/unitCosts.json').categories) || {};
  warn('config/unitCosts.json', known.filter((c) => c !== 'other' && !costCats[c]));
} catch (err) {
  console.error('Category consistency check failed to run:', err.message || err);
}

// Abandoned drafts would otherwise sit on the disk forever.
try {
  const swept = qaDraftStore.sweepOldDrafts(30);
  if (swept) console.log(`Swept ${swept} abandoned QA draft folder(s) older than 30 days.`);
} catch (err) {
  console.error('Draft sweep failed:', err.message || err);
}
...
app.post('/api/submit', upload.any(), async (req, res) => {
  try {
    ...
    const filesByField = {};
    /* Multipart files still arrive from reports started before photos moved to
     * upload-on-add, so both paths are supported. */
    for (const f of files) {
      if (!filesByField[f.fieldname]) filesByField[f.fieldname] = [];
      filesByField[f.fieldname].push(f);
    }

    /* The normal path now: photos are already in this report's draft folder,
     * so the submission sent references and we read them off disk. A missing
     * file is skipped rather than failing the whole submission - losing one
     * photo is bad, losing a completed inspection is worse. */
    if (payload.draftId && payload.photoRefs) {
      let missing = 0;
      Object.entries(payload.photoRefs).forEach(([field, refs]) => {
        (refs || []).forEach((r) => {
          const file = qaDraftStore.readPhoto(payload.draftId, { ...r, field });
          if (!file) { missing += 1; return; }
          if (!filesByField[field]) filesByField[field] = [];
          filesByField[field].push(file);
        });
      });
      if (missing) {
        console.error(`Submission ${payload.poNumber}: ${missing} draft photo(s) could not be read from disk.`);
      }
    }
    ...
    submissionLog.appendSubmission({
      ...
      /* What actually happened to the defective units, from the disposition
       * step. Analytics previously derived its defective rate from the AQL
       * recap's quantityRejected, which is min(checked, major+critical) - an
       * estimate of how many units the sampling plan implies, not how many
       * were really pulled. Now that inspectors record the decision, use it. */
      dispositionSummary: (() => {
        const list = (payload.inspection && payload.inspection.dispositions) || [];
        const sum = { rejected: 0, repairedOnSite: 0, factoryToFix: 0 };
        list.forEach((d) => {
          const flagged = parseInt(d.unitsFlagged, 10) || 0;
          if (d.choice === 'rejected') sum.rejected += flagged;
          else if (d.choice === 'repaired') sum.repairedOnSite += Math.min(flagged, parseInt(d.unitsRepaired, 10) || 0);
          else if (d.choice === 'factory') sum.factoryToFix += flagged;
        });
        return sum;
      })(),
...
/* A Revised Unit Report: the follow-up confirming that units flagged by an
 * earlier report have since been repaired. Deliberately lightweight - it
 * records what was fixed against the original submission rather than being a
 * second full inspection. */
app.post('/api/submit-revised', (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload.poNumber || !Array.isArray(payload.issues) || !payload.issues.length) {
      return res.status(400).json({ error: 'A PO number and at least one confirmed issue are required' });
    }
    // Copy the evidence out of the draft folder so it survives the sweep.
    const stored = payload.issues.map((iss) => ({
      ...iss,
      photos: (iss.photos || []).map((ref) => {
        const file = qaDraftStore.readPhoto(payload.draftId, ref);
        return file ? submissionLog.saveRevisedPhoto(payload.poNumber, file) : null;
      }).filter(Boolean)
    }));
...
app.get('/api/analytics/vendor', (req, res) => {
  try {
    ...
    res.json(analytics.vendorStats(all, req.query.creator, start, end, submissionLog.findRevisedReports()));
  } catch (err) {
...
app.get('/api/analytics/factory', (req, res) => {
  try {
    ...
    const { start, end } = parseDateRange(req.query);
    const all = submissionLog.getAllSubmissions();
    res.json(analytics.factoryStats(all, req.query.factoryCode, start, end, submissionLog.findRevisedReports()));
  } catch (err) {
...
app.get('/api/analytics/category', (req, res) => {
  try {
    const { start, end } = parseDateRange(req.query);
    const all = submissionLog.getAllSubmissions();
    res.json({ categories: analytics.categoryStats(all, start, end, submissionLog.findRevisedReports()) });
  } catch (err) {
...
app.get('/api/order-management/orders/:id', (req, res) => {
  ...
  const viewer = (previewAs && userStore.can(req.user, 'dispatch:send'))
    ? { role: 'supplier', supplierName: String(previewAs) }
    : req.user;
  /* Revised Unit Reports live in their own store (they are follow-ups, not
   * inspections, so analytics must not count them as extra reports). Attached
   * here rather than held on the order record, so the two can't drift. */
  const redacted = userStore.redactOrderForSupplier(viewer, order);
  let revisedReports = [];
  try {
    revisedReports = submissionLog.findRevisedReports(order.poNumber) || [];
  } catch (err) {
    console.error('Could not load revised reports for', order.poNumber, err.message || err);
  }
  res.json({ order: { ...redacted, revisedReports } });
});
          ...
          (async () => {
            try {
              ...
              const buffer = await buildConsolidatedReport(po, fullApproval, reportingHistory, i18n,
                submissionLog.findRevisedReports(po.poNumber));
...
app.get('/api/reports/by-sku/:sku', (req, res) => {
  try {
    /* Include what each PO actually has on it. Without this the list gave no
     * sign that a PO had inspection reports at all, let alone follow-up
     * revisions - so "Download Full Report" was a guess. */
    const pos = orderManagementStore.getOrdersBySku(req.params.sku).map((o) => {
      const shaped = orderManagementStore.toQaShape(o);
      let inspections = 0;
      let revisions = 0;
      try {
        inspections = (submissionLog.findPriorReportsByPoNumber(shaped.poNumber) || []).length;
        revisions = (submissionLog.findRevisedReports(shaped.poNumber) || []).length;
      } catch (err) {
        console.error('Could not count reports for', shaped.poNumber, err.message || err);
      }
      return { ...shaped, reportCounts: { inspections, revisions } };
    });
    res.json({ pos });
  } catch (err) {
...
app.get('/api/consolidated-report/:poNumber', async (req, res) => {
  try {
    ...
    const buffer = await buildConsolidatedReport(po, approval, reportingHistory, i18n, revisedReports);
