    ...
    return `
      <div class="card ${done ? 'revised-done' : ''}" data-revised-idx="${idx}">
        ...
        ${done ? `
          ...
          ${(iss.newPhotos || []).length ? `
            <div class="q-reference">
              <div class="q-reference-label">${escapeHtml(bi('repairEvidenceLabel', 'Repair evidence').en)}</div>
              <div style="display:flex; gap:8px; flex-wrap:wrap;">
                ${iss.newPhotos.map((f) => `<div class="q-reference-frame"><img src="${escapeHtml(f.url || '')}" class="js-lightbox" alt="" /></div>`).join('')}
              </div>
            </div>` : ''}
          ${iss.comment ? `<div class="om-checkin-lastnote" style="margin-top:8px;">${escapeHtml(iss.comment)}</div>` : ''}
          <div class="revised-confirmed-row">
